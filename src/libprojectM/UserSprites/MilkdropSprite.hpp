...
private:
    ...
    Renderer::TextureSamplerDescriptor m_texture; //!< The sprite image, loaded via a name like other textures in Milkdrop presets.
    uint32_t m_colorKey{}; //!< Transparent color key. Turns this exact color into transparency when rendering.
    bool m_enableColorKey{true}; //!< Original Milkdrop wasn't able to disable the color key, but we're providing a setting.
